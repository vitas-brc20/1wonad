...
export async function GET(req) {
  ...
  if (!pg_token || !partner_order_id) {
    // 필수 파라미터가 없는 경우 에러 페이지나 홈페이지로 리다이렉트
  ...
  try {
    ...
    if (fetchError || !messageData) {
      console.error('Approval error: Could not find order.', fetchError);
    ...
    if (updateError) {
      console.error('Approval error: DB status update failed.', updateError);
    ...
    console.error('KakaoPay approve error:', error.response ? error.response.data : error.message);
